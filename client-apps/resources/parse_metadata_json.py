...
def parse_and_write_metadata(product, f_products, f_rankings):
    """
    ...
    num_stars = (random.random() * 2 * num_reviews + 3 * num_reviews)
    avg_stars = num_stars / num_reviews
    ...
    f_products.write(",")
    if "price" in product:
        f_products.write(str(product.get("price", "")))
    f_products.write(",")
    if "imUrl" in product:
...
with open(metadata_file) as f:
    for line in f:
        ...
        parse_and_write_metadata(product, f_products, f_rankings)
